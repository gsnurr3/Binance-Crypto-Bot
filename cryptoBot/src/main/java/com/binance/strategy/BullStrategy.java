...
public class BullStrategy {
    ...
    public PotentialWinningCoin compareHighPriceRecordFirstIndexAndLastIndexCalendarInstance(
            PotentialWinningCoin potentialWinningCoin) {

        Date d1 = potentialWinningCoin.getHighPriceRecords().get(0).getTimeStamp().getTime();
        Date d2 = potentialWinningCoin.getHighPriceRecords().get(potentialWinningCoin.getHighPriceRecords().size() - 1)
        ...
        try {
            ...
            LOGGER.info("High price records were recorded over (seconds): " + diffSeconds);
        } catch (Exception e) {
            emailHandler.sendEmail("Error", "Issue comparing dates in bull strategy. Kill and check Binance Crypto Bot ASAP!");
            e.printStackTrace();
        }
